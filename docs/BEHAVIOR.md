...
What the code *does*, not what it should do. Written before the refactor as a baseline,
kept current since.
...
Generated by [`@kviz/importer`](../packages/importer) from the official PDF. Edit the
importer, not the file.

**Alternatives carry no letter prefix.** The 2024 data stored `"A) Den matek."`, which the
app shuffled, so letters appeared out of order.
...
Still present. Documented rather than fixed.
...
3. **`alt="answer"` on every answer image.** An accurate description would give away
   "which picture shows Karlštejn"; `alt="Možnost 1"` identifies without spoiling.
...
5. **`appLocale.resultFilterUnanswered` is unused.** The filter offers only all, correct
   and incorrect.
6. **Two questions have no illustration.** Topic 12 question 2 cites a Pixabay page, not
   a file; topic 30 question 8 cites `Villa_Tugendhat` with no extension. Both are
   answerable from their wording, so they ship without it.

## Fixed since the baseline

Kept because they shaped the code that exists now.
...
Nine of the ten changes are image paths moving into this repository. The tenth is a
correction: *"Jaká bude podle zákona délka zkušební doby paní Novákové?"* was keyed to
**3 měsíce**; the official answer is **4 měsíce**.

42 in and 41 out overstates the churn — questions are matched by text, so rewording reads
as one removal plus one addition.

Pictures are named for their question, `t30-q4-c.jpg` rather than `18alt3.jpg`. The bank
names files by *position*, so renumbering repoints every later one; that is how four
answers came to show portraits of Czech presidents instead of the National Theatre.

One Commons name needed correcting by hand: the layout wrapped it inside a date and lost a
hyphen, printing `Praha_2005-0920_národní_divadlo.jpg`, which 404s. One verified entry in
`pictures.ts`; the picture was opened and checked against its question.

Question count is 300, 10 per topic.
...
No longer applies: every picture is a file in this repository, named after its question.
Kept because it is the reason the importer exists.
...
The remaining 31 were never audited, so the count of affected questions was unknown. A
status-code check cannot detect this, and auditing 32 pictures by eye does nothing about
the 33rd.

Resolved by taking pictures from the Commons files the PDF's own citations name, stored
under names that carry their question.
